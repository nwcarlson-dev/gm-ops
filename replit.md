...
- **Prospect Data:** 337 prospects with detailed bio (height, weight, age, DOB, hometown — 99%+ coverage), consensus rankings from 7+ sources, position-specific skill ratings (0-100 scale, AI-generated from scouting data), scouting reports (65% coverage from NFLMDD/Jeremiah), traits, archetype, projection, and comparisons. `developmentCertainty` is a key field.
- **Data Enrichment Pipeline:** `scripts/enrichment/` contains automated scrapers and AI enrichment tools:
    - `scrape-rankings.js` — Scrapes 7 ranking sources (PFF, CBS, DraftTek, Jeremiah, FantasyPros, NFLMDD, Tankathon). DraftTek scraper also captures height/weight/class year.
