...
- **Player Ratings Generation (v2.0 Multi-Source):** `scripts/build_player_ratings.js` generates 20-80 skill ratings for all 1,293 rostered players using multi-source blending: PFF grades (30%), Madden EA API ratings (25%), nflverse NFL stats (20%), contract value (10%), draft capital (10%), and performance tier (5%). When any source is missing, its weight redistributes proportionally across remaining sources. Source coverage: PFF 93%, Madden 89%, NFL stats 26% (offensive skill only), contract 97%, draft capital 75%, tier 100%. Each player gets individual skill ratings for every position skill (QB has 11, WR has 9, etc.) plus scheme-specific overall ratings. Madden individual stats are mapped to canonical skills via `data/mappings/madden_skill_map.json` (v2.0), influencing per-skill differentiation beyond the base rating. Full spec in DESIGN.md under "Multi-Source Rating Generation."
- **Projected Ceiling Ratings (Planned):** Each player will also receive projected ceiling ratings per skill, producing ceiling overalls and ceiling scheme overalls. Factors: age curve, performance trajectory, draft pedigree, position-specific aging. Impacts `validateTeamNeeds` urgency, CPU draft logic, trade values, and UI display (dual grade format). Full spec in DESIGN.md under "Projected Ceiling Ratings."
- **Staff Generation System (Planned):** Franchise mode will require generating coaching and front office staff. Two sources: (1) Retired player pipeline — players with high Football IQ and Motor ratings can enter coaching/FO roles after retirement, with their playing position determining natural role fits (QBs → OC/HC, LBs → DC, etc.); (2) Generated non-player staff for roles that don't typically come from playing careers. Staff use the same 20-80 rating scale with 6 ratings (schemeKnowledge, playerDevelopment, gameManagement, talentEvaluation, leadership, adaptability) and share the `developmentTier` progression system. Full spec in DESIGN.md under "Staff Generation System."
- **Scheme Fit System:** Matches team needs with prospect attributes based on `scheme_skill_weights.json`. Version 2.0 uses canonical skill keys aligned with DESIGN.md position skills. Supports 7 offensive and 6 defensive schemes with full position coverage.
...
- **NFLverse:** Source for raw contract, roster, depth chart data, and seasonal player statistics (2022-2024).
- **EA Sports Drop API:** `drop-api.ea.com/rating/madden-nfl` — Official EA API providing Madden 26 player ratings (1,957 players, 55 individual stats per player). Fetched via `scripts/enrichment/fetch_madden_api.js`, stored in `data/raw/madden/ea_api_ratings.json`.
