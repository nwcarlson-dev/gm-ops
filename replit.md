...
- **Player Ratings Generation:** `scripts/build_player_ratings.js` generates 20-80 skill ratings for all 1,293 rostered players using PFF grades, position-specific PFF stats, performance tiers, role/age adjustments, and controlled variance. Each player gets individual skill ratings for every position skill (QB has 11, WR has 9, etc.) plus scheme-specific overall ratings calculated against all offensive or defensive schemes. Ratings are stored in `player_database.json` under each player's `ratings` object. **Current limitation:** ratings lean too heavily on PFF data alone. Planned multi-source approach will blend PFF grades, Madden ratings, NFL stats (multi-season), contract value, draft capital, and accolades — with dynamic weight redistribution when any source is missing. Full spec in DESIGN.md under "Multi-Source Rating Generation."
